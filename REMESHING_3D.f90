MODULE REMESHING_3D
    ...
    SUBROUTINE ATTACH_FLUID_CASE_IMPACT_ZONE()
        IMPLICIT NONE
        INTEGER :: I, IPZ, DIR, ITER, K, I1, L, J1
        REAL(8) :: R, ERR, V1(3), V2(3), V3(3), THRESH, R1, R2, R3
        LOGICAL :: B, FLAG, B1, B2, B3
        INTEGER, ALLOCATABLE :: PROJECTION_INDEX(:,:)
        INTEGER :: PROJECTION_INDEX_NUM
        
        FLAG = .FALSE.
        DIR = 1
        
        ALLOCATE(PROJECTION_INDEX(2,SURFACE_FLUID%SURFACE_FACES_NUM))
        PROJECTION_INDEX(:,:) = 0
        PROJECTION_INDEX_NUM = 0
        
        DO I=1,SURFACE_FLUID%SURFACE_FACES_NUM
            IPZ = SURFACE_FLUID%FACE_IMPACT_ZONE(2+1, I)
            IF(IPZ.NE.0) THEN
                CALL RANDOM_NUMBER(ERR)
                ERR = 0.000001 * ABS(ERR)
                
                CALL DISTANCE_FACE_FACE_TYPE(I,0,IPZ,2,DIR,  R1,B, 1.-2.*ERR, ERR, ERR)
                CALL DISTANCE_FACE_FACE_TYPE(I,0,IPZ,2,DIR,  R2,B, ERR, 1.-2.*ERR, ERR)
                CALL DISTANCE_FACE_FACE_TYPE(I,0,IPZ,2,DIR,  R3,B, ERR, ERR, 1.-2.*ERR)
                
                R = MAX(R1,R2,R3)
                
                V1 = SURFACE_FLUID%SURFACE_POINTS(:,SURFACE_FLUID%SURFACE_FACES(2,I)) - SURFACE_FLUID%SURFACE_POINTS(:,SURFACE_FLUID%SURFACE_FACES(1,I))
                V2 = SURFACE_FLUID%SURFACE_POINTS(:,SURFACE_FLUID%SURFACE_FACES(3,I)) - SURFACE_FLUID%SURFACE_POINTS(:,SURFACE_FLUID%SURFACE_FACES(2,I))
                V3 = SURFACE_FLUID%SURFACE_POINTS(:,SURFACE_FLUID%SURFACE_FACES(1,I)) - SURFACE_FLUID%SURFACE_POINTS(:,SURFACE_FLUID%SURFACE_FACES(3,I))
                
                THRESH = (SQRT(DOT_PRODUCT(V1,V1)) + SQRT(DOT_PRODUCT(V2,V2)) + SQRT(DOT_PRODUCT(V3,V3)))/3.
                
                IF(R < THRESH/100. .AND. SURFACE_FLUID%FACE_ONINTERFACE(I) .NE. 2) THEN
                    FLAG = .TRUE.
                    EXIT
                END IF
            END IF
        ...
        IF(FLAG) THEN
            DO I=1,SURFACE_FLUID%SURFACE_FACES_NUM
                ...
                IF(IPZ.NE.0) THEN
                    ...
                    IF(R < THRESH/10. .AND. SURFACE_FLUID%FACE_ONINTERFACE(I) .NE. 2) THEN
                        SURFACE_FLUID%FACE_ONINTERFACE(I) = 2
                        
                        PROJECTION_INDEX_NUM = PROJECTION_INDEX_NUM + 1
                        
                        PROJECTION_INDEX(1,PROJECTION_INDEX_NUM) = I
                        PROJECTION_INDEX(2,PROJECTION_INDEX_NUM) = IPZ
            ...
            DO ITER=1,PROJECTION_INDEX_NUM
                I = PROJECTION_INDEX(1,ITER)
                IPZ = PROJECTION_INDEX(2,ITER)
                
                DO K=1,3
                    CALL FACE_NEIGHBOR_FACE(SURFACE_FLUID%SURFACE_FACES_NUM, SURFACE_FLUID%SURFACE_FACES, SURFACE_FLUID%POINT_FACE_CONNECTION_NUM, SURFACE_FLUID%POINT_FACE_CONNECTION,I,K, I1)
                ...
                !CALL PROJECTION_FACE_POINT(SURFACE_FLUID%SURFACE_POINTS(:,SURFACE_FLUID%SURFACE_FACES(1,I)), SURFACE_CASE%SURFACE_POINTS(:,SURFACE_CASE%SURFACE_FACES(1,IPZ)), SURFACE_CASE%SURFACE_POINTS(:,SURFACE_CASE%SURFACE_FACES(2,IPZ)), SURFACE_CASE%SURFACE_POINTS(:,SURFACE_CASE%SURFACE_FACES(3,IPZ)))
                
                !CALL PROJECTION_FACE_POINT(SURFACE_FLUID%SURFACE_POINTS(:,SURFACE_FLUID%SURFACE_FACES(2,I)), SURFACE_CASE%SURFACE_POINTS(:,SURFACE_CASE%SURFACE_FACES(1,IPZ)), SURFACE_CASE%SURFACE_POINTS(:,SURFACE_CASE%SURFACE_FACES(2,IPZ)), SURFACE_CASE%SURFACE_POINTS(:,SURFACE_CASE%SURFACE_FACES(3,IPZ)))
                
                !CALL PROJECTION_FACE_POINT(SURFACE_FLUID%SURFACE_POINTS(:,SURFACE_FLUID%SURFACE_FACES(3,I)), SURFACE_CASE%SURFACE_POINTS(:,SURFACE_CASE%SURFACE_FACES(1,IPZ)), SURFACE_CASE%SURFACE_POINTS(:,SURFACE_CASE%SURFACE_FACES(2,IPZ)), SURFACE_CASE%SURFACE_POINTS(:,SURFACE_CASE%SURFACE_FACES(3,IPZ)))
            END DO
            ...
            DO I=1,SURFACE_FLUID%SURFACE_POINTS_NUM
                IF(SURFACE_FLUID%POINT_TYPE(I)<=3 .OR. SURFACE_FLUID%POINT_TYPE(I)==5) THEN
                    
                    B1 = .FALSE.
                    B2 = .FALSE.
                    B3 = .FALSE.
                    DO ITER = 1, SURFACE_FLUID%POINT_FACE_CONNECTION_NUM(I)	
                        J1 = SURFACE_FLUID%POINT_FACE_CONNECTION(ITER,I)
                        IF(SURFACE_FLUID%FACE_ONINTERFACE(J1)==1) THEN
                            B1 = .TRUE.
                        ELSE IF(SURFACE_FLUID%FACE_ONINTERFACE(J1)==2) THEN
                            B2 = .TRUE.
                        ELSE IF(SURFACE_FLUID%FACE_ONINTERFACE(J1)==-1) THEN
                            B3 = .TRUE.
                        END IF
                    END DO
                    
                    IF(B1 .AND. B2 .AND. B3) THEN
                        SURFACE_FLUID%POINT_TYPE(I) = 6
                    ELSE IF((B1 .OR. B2) .AND. B3) THEN
                        SURFACE_FLUID%POINT_TYPE(I) = 5
                    ELSE IF(B1 .AND. B2) THEN
                        SURFACE_FLUID%POINT_TYPE(I) = 4
                    END IF
                    
                ELSE IF(SURFACE_FLUID%POINT_TYPE(I)>=4) THEN
                    B1 = .FALSE.
                    B2 = .FALSE.
                    DO ITER = 1, SURFACE_FLUID%POINT_FACE_CONNECTION_NUM(I)	
                        J1 = SURFACE_FLUID%POINT_FACE_CONNECTION(ITER,I)
                        IF(SURFACE_FLUID%FACE_ONINTERFACE(J1)==1) THEN
                            B1 = .TRUE.
                        ELSEIF(SURFACE_FLUID%FACE_ONINTERFACE(J1)==2) THEN
                            B2 = .TRUE.
                        ELSE
                        END IF
                    END DO
                    ...
                    IF(.NOT. (B1 .AND. B2)) THEN
                        IF(SURFACE_FLUID%POINT_TYPE(I)==6) THEN
                            SURFACE_FLUID%POINT_TYPE(I) = 2
                        ELSE
                            SURFACE_FLUID%POINT_TYPE(I) = 1
                        END IF
                    END IF
                END IF
	...
	B = .TRUE.
        ...
        DO I=1,SURFACE_PROPEL%SURFACE_FACES_NUM
            ...
            IF(IPZ.NE.0) THEN
                ...
                IF(R < 0.007) THEN !SURFACE_FLUID%MESH_SIZE * 6.) THEN
                    TEMPFLAG = .TRUE.
                    EXIT
                END IF
            END IF
		...
		!end if
            ...
            DO K=1,PAIR_NUM
                ...
                IF(MATCH_INDEX(K,2) .NE. 0) THEN
                   IF(R < 0.007) THEN !SURFACE_FLUID%MESH_SIZE * 8.) THEN
	   		...
	   		!SURFACE_PROPEL%SURFACE_PATCHES_TOPCHANGE_TYP(SURFACE_PROPEL%FACE_LOCATION(MATCH_INDEX(K,1))) = 2
			FLAG = .TRUE.
			EXIT
                    END IF
                END IF
    ...
    SUBROUTINE MESH_QUALITY_PROCESS(TYP, FLAG, FLAG_ARRAY)
	IMPLICIT NONE
        INTEGER :: TYP
        LOGICAL :: FLAG
        ...
        !CALL MESH_QUALITY_SQUARE(SURFACE_CURRENT%SURFACE_POINTS_NUM, SURFACE_CURRENT%SURFACE_POINTS, FACE_NUM, FACE, INITIAL_AREA, QUALITY_ARRAY)
        CALL MESH_QUALITY_SQUARE(SURFACE_CURRENT%SURFACE_POINTS_NUM, SURFACE_CURRENT%SURFACE_POINTS, FACE_NUM, FACE, INITIAL_AREA, QUALITY_ARRAY, FILE_NUM = SURFACE_PRESSURE_ITER, TYP = 1)   
     
        DO I=1,FACE_NUM
            IF(QUALITY_ARRAY(I) < SURFACE_CURRENT%SURFACE_INITIAL_MESH_QUALITY(I) / 2.) THEN !0.4) THEN
                FLAG = .TRUE.
                FLAG_ARRAY(SURFACE_CURRENT%FACE_LOCATION(2*I)) = 1
    ...
    SUBROUTINE MERGE_POINT_NEIGHBOR_FACE(FACE_NUM, FACE, CONNECTION_NUM, CONNECTION, P2, P4, J0, J1)
	IMPLICIT NONE
        INTEGER :: FACE_NUM
        INTEGER :: FACE(:,:)
        INTEGER :: CONNECTION_NUM(:)
    ...
    SUBROUTINE INSERT_POINT_NEIGHBOR_FACE(FACE_NUM, FACE, CONNECTION_NUM, CONNECTION, I0, J_INSERT, J_NEIGHBOR)
	IMPLICIT NONE
        INTEGER :: FACE_NUM
        INTEGER :: FACE(:,:)
        INTEGER :: CONNECTION_NUM(:)
    ...
    SUBROUTINE REMOVE_POINT_NEIGHBOR_FACE(FACE_NUM, FACE, CONNECTION_NUM, CONNECTION, I0, J_REMOVE)
	IMPLICIT NONE
        INTEGER :: FACE_NUM
        INTEGER :: FACE(:,:)
        INTEGER :: CONNECTION_NUM(:)
    ...
    SUBROUTINE CHANGE_POINT_NEIGHBOR_FACE(FACE_NUM, FACE, CONNECTION_NUM, CONNECTION, I0, J_BEFORE, J_AFTER)
	IMPLICIT NONE
        INTEGER :: FACE_NUM
        INTEGER :: FACE(:,:)
        INTEGER :: CONNECTION_NUM(:)
    ...
    SUBROUTINE CHANGE_POINT_INDEX(FACE_NUM, FACE, CONNECTION_NUM, CONNECTION, I_BEFORE, I_AFTER)
	IMPLICIT NONE
        INTEGER :: FACE_NUM
        INTEGER :: FACE(:,:)
        INTEGER :: CONNECTION_NUM(:)
    ...
    SUBROUTINE CHANGE_FACE_INDEX(FACE_NUM, FACE, CONNECTION_NUM, CONNECTION, J_BEFORE, J_AFTER)
	IMPLICIT NONE
        INTEGER :: FACE_NUM
        INTEGER :: FACE(:,:)
        INTEGER :: CONNECTION_NUM(:)
    ...
    SUBROUTINE EDGE_SPLITTING_PROCESS_ONE(TEMP_POINT_NUM, TEMPPOINT, TEMP_FACE_NUM, TEMPFACE, TEMPPOINT_TYPE, TEMP_NEWONINTERFACE, &
	...
	IMPLICIT NONE
 	...
 	INTEGER :: TEMP_ABLATION_FLAG(:)
        ...
        INTEGER, OPTIONAL :: POINT_NEW1, POINT_NEW2, POINT_NEW3, POINT_NEW4, POINT_NEW5
                            ...
                            CALL FACE_NEIGHBOR_FACE(TEMP_FACE_NUM, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION,I,K, I1)
                            ...
                            TEMP_NEWEDGELENGTH(1,I) = EDGELENGTH14
                            TEMP_NEWEDGELENGTH(2,I) = EDGELENGTH12
                            TEMP_NEWEDGELENGTH(3,I) = EDGELENGTH24
                            TEMP_NEWEDGELENGTH(1,I1) = EDGELENGTH23
                            ...
                            TEMP_NEWEDGELENGTH(3,TEMP_FACE_NUM-1) = EDGELENGTH34
                            TEMP_NEWEDGELENGTH(1,TEMP_FACE_NUM) = EDGELENGTH24
                            TEMP_NEWEDGELENGTH(2,TEMP_FACE_NUM) = EDGELENGTH14
                            TEMP_NEWEDGELENGTH(3,TEMP_FACE_NUM) = EDGELENGTH12
                            ...
                            CALL INSERT_POINT_NEIGHBOR_FACE(TEMP_FACE_NUM, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, POINT_IDX1, TEMP_FACE_NUM, I)
                            CALL INSERT_POINT_NEIGHBOR_FACE(TEMP_FACE_NUM, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, POINT_IDX3, TEMP_FACE_NUM-1, I1)
                            
                            CALL CHANGE_POINT_NEIGHBOR_FACE(TEMP_FACE_NUM, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, POINT_IDX2, I1, TEMP_FACE_NUM-1)
                            CALL CHANGE_POINT_NEIGHBOR_FACE(TEMP_FACE_NUM, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, POINT_IDX4, I, TEMP_FACE_NUM)
    ...
    SUBROUTINE EDGE_COLLAPSING_PROCESS_ONE(TEMP_POINT_NUM, TEMPPOINT, TEMP_FACE_NUM, TEMPFACE, TEMPPOINT_TYPE, TEMP_NEWONINTERFACE, &
 	...
 	INTEGER :: TEMP_ABLATION_FLAG(:)
        ...
        INTEGER, OPTIONAL :: POINT_NEW1, POINT_NEW2, POINT_NEW3
                            ...
                            IF(PRESENT(DIVIDED_BOUNDARY_ARRAY)) THEN
                                ...
                                CALL FACE_NEIGHBOR_FACE(TEMP_FACE_NUM,TEMPFACE,TEMP_CONNECTION_NUM,TEMP_CONNECTION,I,MOD(K+3-2, 3) + 1, II1)
                                CALL FACE_NEIGHBOR_FACE(TEMP_FACE_NUM,TEMPFACE,TEMP_CONNECTION_NUM,TEMP_CONNECTION,I1,L, II2)
                                CALL FACE_NEIGHBOR_FACE(TEMP_FACE_NUM,TEMPFACE,TEMP_CONNECTION_NUM,TEMP_CONNECTION,I1,MOD(L,3) + 1, II3)
                                CALL FACE_NEIGHBOR_FACE(TEMP_FACE_NUM,TEMPFACE,TEMP_CONNECTION_NUM,TEMP_CONNECTION,I,MOD(K,3) + 1, II4)
                            ...
                            CALL CHANGE_POINT_INDEX(TEMP_FACE_NUM, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, POINT_IDX4, POINT_IDX2)
                            CALL REMOVE_POINT_NEIGHBOR_FACE(TEMP_FACE_NUM, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, POINT_IDX1, I)
                            CALL REMOVE_POINT_NEIGHBOR_FACE(TEMP_FACE_NUM, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, POINT_IDX3, I1)
                            CALL MERGE_POINT_NEIGHBOR_FACE(TEMP_FACE_NUM, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, POINT_IDX2, POINT_IDX4, I, I1)
                            
                            IF(POINT_IDX4 .NE. TEMP_POINT_NUM) THEN
                                CALL CHANGE_POINT_INDEX(TEMP_FACE_NUM, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, TEMP_POINT_NUM, POINT_IDX4)
                            ...
                            IF(I .NE. TEMP_FACE_NUM) THEN
                                CALL CHANGE_FACE_INDEX(TEMP_FACE_NUM, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, TEMP_FACE_NUM, I)
                            ...
                            IF(I1 .NE. TEMP_FACE_NUM-1) THEN
                                CALL CHANGE_FACE_INDEX(TEMP_FACE_NUM, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, TEMP_FACE_NUM-1, I1)
    ...
    SUBROUTINE EDGE_FLIPPING_PROCESS_ONE(TEMP_FACE_NUM, TEMPFACE, TEMP_NEWEDGELENGTH, TEMP_NEWFACEAREA, TEMP_CONNECTION, TEMP_CONNECTION_NUM, I, K, DIVIDED_BOUNDARY_ARRAY)
	IMPLICIT NONE
        ...
        REAL(8) :: R
                            ...
                            CALL INSERT_POINT_NEIGHBOR_FACE(TEMP_FACE_NUM, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, POINT_IDX1, I1, I)
                            CALL INSERT_POINT_NEIGHBOR_FACE(TEMP_FACE_NUM, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, POINT_IDX3, I, I1)
                            CALL REMOVE_POINT_NEIGHBOR_FACE(TEMP_FACE_NUM, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, POINT_IDX2, I1)
                            CALL REMOVE_POINT_NEIGHBOR_FACE(TEMP_FACE_NUM, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, POINT_IDX4, I)
    
    END SUBROUTINE EDGE_FLIPPING_PROCESS_ONE
    
    SUBROUTINE CHECK_FACE_REMESHING_AVAILABLE(FACE_NUM, FACE, CONNECTION_NUM, CONNECTION, I0, REMESHING_EDGE,        B)
	IMPLICIT NONE
        INTEGER :: FACE_NUM
        INTEGER :: FACE(:,:)
        INTEGER :: CONNECTION_NUM(:)
        ...
        DO K=1,3
            ...
            CALL FACE_NEIGHBOR_FACE(FACE_NUM, FACE, CONNECTION_NUM, CONNECTION,I0,K, I1)
            DO L=1,3
                CALL FACE_NEIGHBOR_FACE(FACE_NUM, FACE, CONNECTION_NUM, CONNECTION,I1,L, I2)
                IF(I2==I0) THEN
                    EXIT
                END IF
            END DO
    ...
    SUBROUTINE QUALITY_LEAST_SQUARE(POINT_NUM, POINT, FACE_NUM, FACE, INITIAL_AREA,        S)
	IMPLICIT NONE
        INTEGER :: POINT_NUM
        REAL(8) :: POINT(:,:)
        INTEGER :: FACE_NUM
            ...
            DO ITER=1,BOUNDARY_NUM
                ...
                DO J=1,CONNECTION_NUM(I)
                    ...
                    IF(DIVIDED_REGION_ARRAY(CONNECTION(J,I)) .NE. DIVIDED_REGION_FLAG) THEN
                        C = .FALSE.
                    ELSE
                        CALL POINT_NEIGHBOR_POINT(FACE_NUM, FACE, CONNECTION_NUM, CONNECTION, I, MOD(J,CONNECTION_NUM(I))+1, I1)
                        CALL POINT_NEIGHBOR_POINT(FACE_NUM, FACE, CONNECTION_NUM, CONNECTION, I, J, I2)
    ...
    SUBROUTINE LOCAL_REGION_FLIPPING(TEMP_POINT_NUM, TEMPPOINT, TEMP_FACE_NUM, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, TEMPPOINT_TYPE, TEMP_NEWEDGELENGTH, TEMP_NEWFACEAREA, LOCAL_REGION_ARRAY, LOCAL_REGION_NUM, IS_INTERIOR_POINT, LOCAL_REGION_FACES, DIVIDED_BOUNDARY_ARRAY, FLAG)
	IMPLICIT NONE
        ...
        LOGICAL :: B,C
        INTEGER :: ITER, I, K, T, I1, POINT_IDX1, POINT_IDX2, POINT_IDX3, POINT_IDX4, DEG_BEFORE, DEG_AFTER, DEG_TOTAL, SMOOTHING_NUM
        REAL(8) :: S, S1, F_SIZE_SHAPE, F_SIZE, F_SHAPE, F_SIZE_SHAPE1, F_SIZE1, F_SHAPE1, S_NEW, S1_NEW, F_SIZE_SHAPE_NEW, F_SIZE_NEW, F_SHAPE_NEW, F_SIZE_SHAPE1_NEW, F_SIZE1_NEW, F_SHAPE1_NEW, QUALITY_BEFORE, QUALITY_AFTER, QUALITY_TOTAL, N(3), N1(3), N_AVG(3), N_NEW(3), N1_NEW(3)
        ...
        DO WHILE(B .AND. LL<30)
            ...
            DO I=1,SMOOTHING_NUM
                CALL AREA_BASED_NULLSPACE_SMOOTHING(TEMP_POINT_NUM, TEMPPOINT, TEMP_FACE_NUM, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, TEMPPOINT_TYPE, TEMP_NEWFACEAREA, IS_INTERIOR_POINT)
            END DO
            ...
            DO ITER=1,LOCAL_REGION_NUM
                I = LOCAL_REGION_ARRAY(ITER)
                    DO K=1,3
                        ...
                        DO T=1,3
                            IF(TEMPFACE(T,I1)==POINT_IDX2) THEN
                                POINT_IDX3 = TEMPFACE(MOD(T,3) + 1, I1)
                                EXIT
                            END IF
                        END DO
                        ...
                        DEG_BEFORE = (TEMP_CONNECTION_NUM(POINT_IDX1)-6)**2 + (TEMP_CONNECTION_NUM(POINT_IDX2)-6)**2 + (TEMP_CONNECTION_NUM(POINT_IDX3)-6)**2 + (TEMP_CONNECTION_NUM(POINT_IDX4)-6)**2
                        DEG_AFTER = (TEMP_CONNECTION_NUM(POINT_IDX1)+1-6)**2 + (TEMP_CONNECTION_NUM(POINT_IDX2)-1-6)**2 + (TEMP_CONNECTION_NUM(POINT_IDX3)+1-6)**2 + (TEMP_CONNECTION_NUM(POINT_IDX4)-1-6)**2
                        
                        S = TEMP_NEWFACEAREA(I)
                        S1 = TEMP_NEWFACEAREA(I1)
                        ...
                        QUALITY_BEFORE = 1./(F_SHAPE+MINERROR) + 1./(F_SHAPE1+MINERROR) !(F_SHAPE-1.)**2 + (F_SHAPE1-1.)**2
                        QUALITY_AFTER = 1./(F_SHAPE_NEW+MINERROR) + 1./(F_SHAPE1_NEW+MINERROR) !(F_SHAPE_NEW-1.)**2 + (F_SHAPE1_NEW-1.)**2
                        
                        CALL SPLITTING_AVAILABLE(TEMP_POINT_NUM, TEMPPOINT, TEMP_FACE_NUM, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, TEMPPOINT_TYPE, DIVIDED_BOUNDARY_ARRAY, I, K, I1,    C)
                        
                        !IF(DEG_AFTER<DEG_BEFORE) THEN
                        !IF(DEG_AFTER<DEG_BEFORE .OR. QUALITY_AFTER*2.<QUALITY_BEFORE) THEN
                        IF(QUALITY_AFTER<QUALITY_BEFORE .AND. TEMP_CONNECTION_NUM(POINT_IDX2)>4 .AND. TEMP_CONNECTION_NUM(POINT_IDX4)>4 .AND. TEMP_CONNECTION_NUM(POINT_IDX1)<8 .AND. TEMP_CONNECTION_NUM(POINT_IDX3)<8 .AND. C) THEN
                            !B = .FALSE.
                            IF(PRESENT(DIVIDED_BOUNDARY_ARRAY)) THEN
                                IF(DIVIDED_BOUNDARY_ARRAY(K,I)==0) THEN
                                    B = .TRUE.
                                    CALL EDGE_FLIPPING_PROCESS_ONE(TEMP_FACE_NUM, TEMPFACE, TEMP_NEWEDGELENGTH, TEMP_NEWFACEAREA, TEMP_CONNECTION, TEMP_CONNECTION_NUM, I, K, DIVIDED_BOUNDARY_ARRAY = DIVIDED_BOUNDARY_ARRAY)
                                END IF
                            ELSE
                                B = .TRUE.
                                CALL EDGE_FLIPPING_PROCESS_ONE(TEMP_FACE_NUM, TEMPFACE, TEMP_NEWEDGELENGTH, TEMP_NEWFACEAREA, TEMP_CONNECTION, TEMP_CONNECTION_NUM, I, K)
                            END IF
                            ...
                            CALL QUALITY_LEAST_SQUARE(TEMP_POINT_NUM, TEMPPOINT, TEMP_FACE_NUM, TEMPFACE, TEMP_NEWFACEAREA, QUALITY_TOTAL)
	...
	IMPLICIT NONE
        ...
        CALL AREA_DIVIDING(FACE_NUM, FACE, CONNECTION_NUM, CONNECTION, POINT_TYPE, DIVIDED_REGION_ARRAY, DIVIDED_REGION_NUM, DIVIDED_BOUNDARY_ARRAY)
    ...
    SUBROUTINE FLIPPING_AVAILABLE(TEMP_POINT_NUM, TEMPPOINT, TEMP_FACE_NUM, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, TEMP_NEWFACEAREA, I, K, I1,    B)
	IMPLICIT NONE
        INTEGER :: TEMP_POINT_NUM
        REAL(8) :: TEMPPOINT(:,:)
        INTEGER :: TEMP_FACE_NUM
        ...
        DO L=1, TEMP_CONNECTION_NUM(POINT_IDX1)
            ...
            CALL MESH_QUALITY_TRIANGLE_ONE(TEMPPOINT(:,TEMPFACE(1,II)), TEMPPOINT(:,TEMPFACE(2,II)), TEMPPOINT(:,TEMPFACE(3,II)), TEMP_NEWFACEAREA(II), F_SIZE_SHAPE, F_SIZE, F_SHAPE)
        ...
        DO L=1, TEMP_CONNECTION_NUM(POINT_IDX1)
            ...
            IF(II.NE.I) THEN
                IF(TEMPFACE(1,II)==POINT_IDX1) THEN
                    CALL MESH_QUALITY_TRIANGLE_ONE(NEWPOINT, TEMPPOINT(:,TEMPFACE(2,II)), TEMPPOINT(:,TEMPFACE(3,II)), TEMP_NEWFACEAREA(II), F_SIZE_SHAPE, F_SIZE, F_SHAPE)
                ELSE IF(TEMPFACE(2,II)==POINT_IDX1) THEN
                    CALL MESH_QUALITY_TRIANGLE_ONE(TEMPPOINT(:,TEMPFACE(1,II)), NEWPOINT, TEMPPOINT(:,TEMPFACE(3,II)), TEMP_NEWFACEAREA(II), F_SIZE_SHAPE, F_SIZE, F_SHAPE)
                ELSE
                    CALL MESH_QUALITY_TRIANGLE_ONE(TEMPPOINT(:,TEMPFACE(1,II)), TEMPPOINT(:,TEMPFACE(2,II)), NEWPOINT, TEMP_NEWFACEAREA(II), F_SIZE_SHAPE, F_SIZE, F_SHAPE)
                END IF
            END IF
            QUALITY_AFTER = QUALITY_AFTER + 1./(F_SHAPE+MINERROR)**2
        END DO
        
        IF(QUALITY_AFTER < QUALITY_BEFORE) THEN
            B = .TRUE.
        END IF
        
    END SUBROUTINE FLIPPING_AVAILABLE
    
    SUBROUTINE SPLITTING_AVAILABLE(TEMP_POINT_NUM, TEMPPOINT, TEMP_FACE_NUM, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, TEMPPOINT_TYPE, DIVIDED_BOUNDARY_ARRAY, I, K, I1,    B)
	IMPLICIT NONE
        INTEGER :: TEMP_POINT_NUM
        REAL(8) :: TEMPPOINT(:,:)
        INTEGER :: TEMP_FACE_NUM
    ...
    SUBROUTINE COLLAPSING_AVAILABLE(TEMP_POINT_NUM, TEMPPOINT, TEMP_FACE_NUM, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, TEMPPOINT_TYPE, DIVIDED_BOUNDARY_ARRAY, I, K, I1,    B)
	IMPLICIT NONE
        INTEGER :: TEMP_POINT_NUM
        REAL(8) :: TEMPPOINT(:,:)
        INTEGER :: TEMP_FACE_NUM
        ...
        IF(DIVIDED_BOUNDARY_ARRAY(MOD(K,3) + 1, I) == 0 .AND. DIVIDED_BOUNDARY_ARRAY(MOD(K+3-2,3) + 1, I) == 0 .AND. DIVIDED_BOUNDARY_ARRAY(MOD(J,3) + 1, I1) == 0 .AND. DIVIDED_BOUNDARY_ARRAY(J, I1) == 0 .AND. TEMP_CONNECTION_NUM(POINT_IDX1)>=5 .AND. TEMP_CONNECTION_NUM(POINT_IDX3)>=5) THEN
            B = .TRUE.
        ELSE IF(.NOT. (DIVIDED_BOUNDARY_ARRAY(MOD(K,3) + 1, I) .NE. 0 .AND. DIVIDED_BOUNDARY_ARRAY(MOD(K+3-2,3) + 1, I) .NE. 0) .AND. .NOT. (DIVIDED_BOUNDARY_ARRAY(MOD(J,3) + 1, I1) .NE. 0 .AND. DIVIDED_BOUNDARY_ARRAY(J, I1) .NE. 0) .AND. TEMP_CONNECTION_NUM(POINT_IDX1)>=5 .AND. TEMP_CONNECTION_NUM(POINT_IDX3)>=5) THEN
            
            CALL FACE_NEIGHBOR_FACE(TEMP_FACE_NUM,TEMPFACE,TEMP_CONNECTION_NUM,TEMP_CONNECTION,I,MOD(K+3-2, 3) + 1, II1)
            CALL FACE_NEIGHBOR_FACE(TEMP_FACE_NUM,TEMPFACE,TEMP_CONNECTION_NUM,TEMP_CONNECTION,I1,J, II2)
            CALL FACE_NEIGHBOR_FACE(TEMP_FACE_NUM,TEMPFACE,TEMP_CONNECTION_NUM,TEMP_CONNECTION,I1,MOD(J,3) + 1, II3)
            CALL FACE_NEIGHBOR_FACE(TEMP_FACE_NUM,TEMPFACE,TEMP_CONNECTION_NUM,TEMP_CONNECTION,I,MOD(K,3) + 1, II4)
    ...
    SUBROUTINE REMESHING_PROCESS(TYP, FLAG, FLAG_ARRAY)
	IMPLICIT NONE
        INTEGER :: TYP
        LOGICAL :: FLAG
        INTEGER, OPTIONAL :: FLAG_ARRAY(:)
 	...
 	INTEGER, ALLOCATABLE :: TEMP_ABLATION_FLAG(:)
        ...
        INTEGER :: DIVIDED_REGION_NUM
        INTEGER, ALLOCATABLE :: TEMP_DIVIDED_REGION_ARRAY(:)
        ...
        REAL(8) :: V1(3), V2(3), N(3), AREA, R, RMAX, RMIN, V1_1(3), V2_1(3), N1(3), AREA1, LENGTH, F_SHAPE, F_SIZE, F_SIZE_SHAPE
        ...
        DO ITER=1,DIVIDED_BOUNDARY_NUM
            ...
            DO I=1,TEMP_FACE_NUM
                DO K=1,3
                    IF(DIVIDED_BOUNDARY_ARRAY(K,I)==ITER) THEN
                        ...
                        CALL COLLAPSING_AVAILABLE(TEMP_POINT_NUM, TEMPPOINT, TEMP_FACE_NUM, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, TEMPPOINT_TYPE, DIVIDED_BOUNDARY_ARRAY, I, K, I1,    C)
            ...
            IF(ONE_BOUNDARY_LENGTH > ONE_BOUNDARY_INITIAL_LENGTH * 1.05 .OR. ONE_BOUNDARY_LENGTH < ONE_BOUNDARY_INITIAL_LENGTH / 1.05 .OR. RMAX > 2. .OR. RMIN < 0.5) THEN
                B = .TRUE.
            END IF
            ...
            DO WHILE(B)
                ...
                IF(ONE_BOUNDARY_LENGTH > ONE_BOUNDARY_INITIAL_LENGTH + TEMP_NEWEDGELENGTH(KMAX,IMAX) .OR. RMAX > 2.) THEN
                    FLAG = .TRUE.
                    B = .TRUE.
                    ...
                    CALL EDGE_SPLITTING_PROCESS_ONE(TEMP_POINT_NUM, TEMPPOINT, TEMP_FACE_NUM, TEMPFACE, TEMPPOINT_TYPE, TEMP_NEWONINTERFACE, &
                ...
                IF(ONE_BOUNDARY_LENGTH < ONE_BOUNDARY_INITIAL_LENGTH - TEMP_NEWEDGELENGTH(KMIN,IMIN) .OR. RMIN < 0.5) THEN
                    FLAG = .TRUE.
                    B = .TRUE.
                    ...
                    CALL EDGE_COLLAPSING_PROCESS_ONE(TEMP_POINT_NUM, TEMPPOINT, TEMP_FACE_NUM, TEMPFACE, TEMPPOINT_TYPE, TEMP_NEWONINTERFACE, &
                ...
                IF(B) THEN
                    CALL NEW_MODIFIED_NULLSPACE_SMOOTHING(TEMP_POINT_NUM, TEMPPOINT, TEMP_FACE_NUM, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, TEMP_NEWFACEAREA, TEMP_NEWEDGELENGTH, TEMPPOINT_TYPE, DIVIDED_BOUNDARY_ARRAY)
                    ...
                    CALL LOCAL_REGION_FLIPPING(TEMP_POINT_NUM, TEMPPOINT, TEMP_FACE_NUM, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, TEMPPOINT_TYPE, TEMP_NEWEDGELENGTH, TEMP_NEWFACEAREA, LOCAL_REGION_ARRAY, LOCAL_REGION_NUM, IS_INTERIOR_POINT, LOCAL_REGION_FACES, DIVIDED_BOUNDARY_ARRAY = DIVIDED_BOUNDARY_ARRAY)
...
!        END DO
        ...
        DO ITER=1,DIVIDED_REGION_NUM
            ...
            IF(ONE_REGION_AREA > ONE_REGION_INITIAL_AREA * 1.1 .OR. ONE_REGION_AREA < ONE_REGION_INITIAL_AREA / 1.1 .OR. RMAX > 9. .OR. RMIN < 1./9.) THEN
                B = .TRUE.
            END IF
            ...
            DO WHILE(B)
                ...
                IF(ONE_REGION_AREA > ONE_REGION_INITIAL_AREA + TEMP_NEWFACEAREA(IMAX) + TEMP_NEWFACEAREA(I1MAX) .OR. RMAX > 9.) THEN
                    FLAG = .TRUE.
                    B = .TRUE.
                ...
                IF(ONE_REGION_AREA < ONE_REGION_INITIAL_AREA - TEMP_NEWFACEAREA(IMIN) - TEMP_NEWFACEAREA(I1MIN) .OR. RMIN < 1./9.) THEN
                    FLAG = .TRUE.
                    B = .TRUE.
                    ...
                    DO J=1,TEMP_CONNECTION_NUM(POINT_NEW2)
                        IF(DIVIDED_REGION_ARRAY(TEMP_CONNECTION(J,POINT_NEW2))==ITER) THEN
                            BB = .TRUE.
                            EXIT
                        END IF
                    END DO
        ...
        IF(B) THEN
                ...
                IF(DIVIDED_REGION_ARRAY(I)==ITER) THEN
                    ...
                    DO K=1,3
                        ...
                        IF(DIVIDED_REGION_ARRAY(I1).NE.ITER) THEN
                            ...
                            ELSE IF(TEMP_NEWONINTERFACE(I1)==2) THEN
                                REGION2 = DIVIDED_REGION_ARRAY(I1)
                            END IF
                            !IF(TEMPPOINT_TYPE(TEMPFACE(K,I))>=4 .AND. TEMPPOINT_TYPE(TEMPFACE(MOD(K,3)+1,I))>=4) THEN
                            !    REGION3 = DIVIDED_REGION_ARRAY(I1)
                            !ELSE IF(TEMP_NEWONINTERFACE(I1)==-1) THEN
                            !    REGION2 = DIVIDED_REGION_ARRAY(I1)
                            !ELSE
                            !    REGION1 = DIVIDED_REGION_ARRAY(I1)
            ...
            IF(REGION_FACE_NUM <= 8 .OR. ONE_REGION_AREA < ONE_REGION_INITIAL_AREA/3.) THEN
                !IF(REGION1==0 .AND. REGION2==0) THEN
                !    REGION1 = REGION3
                !ELSE IF(REGION1==0) THEN
                !    REGION1 = REGION2
                !END IF
                ...
                DO I=1,TEMP_FACE_NUM
                    ...
                    IF(REGION1 == 0 .AND. REGION2 .NE. 0) THEN
                        TEMP_NEWONINTERFACE(I) = 2
                    END IF
                    DO K=1,3
                        CALL FACE_NEIGHBOR_FACE(TEMP_FACE_NUM, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION,I,K, I1)
                        DO J=1,3
                            ...
                            END IF
                        END DO
                        IF(REGION1 .NE. 0 .AND. DIVIDED_REGION_ARRAY(I1)==REGION1) THEN
                            DIVIDED_BOUNDARY_ARRAY(K,I) = 0
                            DIVIDED_BOUNDARY_ARRAY(MOD(J+3-2,3)+1,I1) = 0
                        ELSE IF(REGION1 == 0 .AND. DIVIDED_REGION_ARRAY(I1)==REGION2) THEN
                            DIVIDED_BOUNDARY_ARRAY(K,I) = 0
                            DIVIDED_BOUNDARY_ARRAY(MOD(J+3-2,3)+1,I1) = 0
                        END IF
                    END DO
        ...
        DO ITER=1,DIVIDED_REGION_NUM
            ...
            DO WHILE(B .AND. L < 30)
                ...
                DO I=1,TEMP_FACE_NUM
                    ...
                    CALL MAKE_LOCAL_REGION(TEMP_POINT_NUM, TEMP_FACE_NUM, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, DIVIDED_REGION_ARRAY, ITER, FLIP_START, LOCAL_REGION_ARRAY, LOCAL_REGION_NUM, IS_INTERIOR_POINT, LOCAL_REGION_FACES)
                    CALL LOCAL_REGION_FLIPPING(TEMP_POINT_NUM, TEMPPOINT, TEMP_FACE_NUM, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, TEMPPOINT_TYPE, TEMP_NEWEDGELENGTH, TEMP_NEWFACEAREA, LOCAL_REGION_ARRAY, LOCAL_REGION_NUM, IS_INTERIOR_POINT, LOCAL_REGION_FACES, DIVIDED_BOUNDARY_ARRAY = DIVIDED_BOUNDARY_ARRAY, FLAG = FLAG)
                    ...
                    DO K=1,3
                        IF(DIVIDED_BOUNDARY_ARRAY(K,I)==ITER .AND. DIVIDED_BOUNDARY_ARRAY(MOD(K,3)+1,I)==0 .AND. DIVIDED_BOUNDARY_ARRAY(MOD(K+3-2,3)+1,I)==0) THEN
                            ...
                            IF(F_SHAPE < 0.4) THEN
                                ...
                                CALL FLIPPING_AVAILABLE(TEMP_POINT_NUM, TEMPPOINT, TEMP_FACE_NUM, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, TEMP_NEWFACEAREA, I, K, I1, C)
                                IF(C) THEN
                                    !FLIP_START = TEMPFACE(1,I)
                                    FLAG = .TRUE.
                                    B = .TRUE.
        ...
        IF(TYP==0 .OR. TYP==1) THEN
            ...
            SURFACE_INITIAL_FACE_AREA = TEMP_NEWFACEAREA, SURFACE_INITIAL_EDGE_LENGTH = TEMP_NEWEDGELENGTH, POINT_TYPE = TEMPPOINT_TYPE, FACE_ONINTERFACE = TEMP_NEWONINTERFACE, &
            FACE_LOCATION = TEMP_NEWLOCATION, FACE_ABLATION_FLAG = TEMP_ABLATION_FLAG, FACE_DIVIDED_REGION_ARRAY = DIVIDED_REGION_ARRAY, FACE_DIVIDED_BOUNDARY_ARRAY = DIVIDED_BOUNDARY_ARRAY)
        ELSE
            CALL RESET_SURFACE(TYP, .TRUE., TEMP_POINT_NUM, TEMP_FACE_NUM, SURFACE_POINTS = TEMPPOINT, SURFACE_FACES = TEMPFACE, &
            SURFACE_INITIAL_FACE_AREA = TEMP_NEWFACEAREA, SURFACE_INITIAL_EDGE_LENGTH = TEMP_NEWEDGELENGTH, POINT_TYPE = TEMPPOINT_TYPE, FACE_ONINTERFACE = TEMP_NEWONINTERFACE, FACE_DIVIDED_REGION_ARRAY = DIVIDED_REGION_ARRAY, FACE_DIVIDED_BOUNDARY_ARRAY = DIVIDED_BOUNDARY_ARRAY)
        END IF
        END IF
        ...
        !CALL MESH_QUALITY_TRIANGLE(SURFACE_CURRENT%SURFACE_POINTS_NUM, SURFACE_CURRENT%SURFACE_POINTS, SURFACE_CURRENT%SURFACE_FACES_NUM, SURFACE_CURRENT%SURFACE_FACES, SURFACE_CURRENT%SURFACE_INITIAL_FACE_AREA, QUALITY_ARRAY, 0, 0)
    ...
    SUBROUTINE AREA_BASED_NULLSPACE_SMOOTHING(TEMP_POINT_NUM, TEMPPOINT, TEMP_FACE_NUM, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, TEMPPOINT_TYPE, TEMP_NEWFACEAREA, IS_INTERIOR_POINT)
	IMPLICIT NONE
        INTEGER :: TEMP_POINT_NUM
        REAL(8) :: TEMPPOINT(:,:)
        INTEGER :: TEMP_FACE_NUM
	...
	REAL(8) :: R
		...
		ORIGIN_W(J) = (1./TEMP_NEWFACEAREA(TEMP_CONNECTION(J,I)))/ORIGIN_W_SUM
            ...
            DO J = 1,CON_NUM
                ...
                CALL POINT_NEIGHBOR_POINT(TEMP_FACE_NUM, TEMPFACE, TEMP_CONNECTION_NUM, TEMP_CONNECTION, I, J,      I1)
